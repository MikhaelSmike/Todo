...
<div class="container mx-auto">
    ...
    <div class="bg-pink-300 w-8/12 mx-auto border my-6 rounded-md">
        <div class="px-4">
            <form action="" method="post" class="rounded px-4 pt-5 pb-4 mb-2">
                @csrf
                <label for="taskname" class="block text-gray-700 font-bold mb-2 mt-2">Task Name</label>
                <input type="text" name="taskname" value="" class="border rounded w-full py-2 px-3 text-gray-700">
                <div class="text-red-600">
                    @error('taskname') {{$message}} @enderror
                </div>

                <label for="description" class="block text-gray-700 font-bold mb-2 mt-2">Description</label>
                <input type="text" name="description" value="" class="border rounded w-full py-8 px-3 text-gray-700">
                <div class="text-red-600">
                    @error('description') {{$message}} @enderror
                </div>

                <label for="duedate" class="block text-gray-700 font-bold mb-2 mt-2">Due Date</label>
                <input type="date" name="duedate" value="" class="border rounded w-full py-2 px-3 text-gray-700">
                <div class="text-red-600">
